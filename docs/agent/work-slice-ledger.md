...
- Slice type: Evidence / Contract
- Before: readiness, release-decision, version-policy, package-smoke, and advisory packets existed, but the current `origin/main` exact SHA, remote CI proof, public raw proof, and report linkage were not pinned in one packet.
...
- Gate delta: exact candidate SHA proof is no longer an unrecorded blocker for current `origin/main`, but it becomes historical if `origin/main` moves and it does not authorize release execution.
