...
def test_executor_bridge_cli_json_reports_artifact_write_failure(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    loop_id, _session_id = prepare_autonomy_session(tmp_path)
    bridge_dir = tmp_path / ".qa-z" / "executor" / "bridge-write-failure"
    original_write_text = Path.write_text

    def fail_bridge_manifest(
        path: Path,
        data: str,
        ...
        if path == bridge_dir / "bridge.json":
            raise OSError("disk full")
        return original_write_text(
            path, data, encoding=encoding, errors=errors, newline=newline
        )

    monkeypatch.setattr(Path, "write_text", fail_bridge_manifest)
