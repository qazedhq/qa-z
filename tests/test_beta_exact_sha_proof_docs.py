...
def test_exact_sha_proof_report_exists_and_records_post_merge_sha() -> None:
    ...
    assert f"Post-merge exact SHA: `{POST_MERGE_SHA}`" in report
...
def test_exact_sha_proof_distinguishes_historical_and_current_proof() -> None:
    report = read(PROOF_REPORT)
    text = normalized(report)

    assert "historical pre-packet proof" in text
    assert "This proof is historical proof only." in report
    assert "current `origin/main` proof basis" in text
    assert (
        "Post-merge exact SHA proof is recorded for the current `origin/main`" in text
    )
    assert (
    ...
    assert "| post-merge exact SHA refresh | `PASS` |" in gate_delta
    ...
    assert "post-merge exact SHA refresh" not in remaining
...
def test_existing_beta_reports_link_current_exact_sha_proof_without_unlocking_release() -> (
    ...
    for report_path in (
        ...
        assert POST_MERGE_SHA in report
