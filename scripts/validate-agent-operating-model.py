...
for pattern in DANGEROUS_CONFIG:
    if re.search(pattern, config_text):
        fail(
            ".codex/config.toml: do not hard-code approval_policy=never or sandbox_mode=danger-full-access in repo config"
        )
...
if "product-code-slice" not in loop_text or "next-real-slices.md" not in loop_text:
    fail(
        "project-improvement-loop skill must hand off to product-code-slice and next-real-slices.md"
    )
...
for path in [ROOT / "AGENTS.md", ROOT / ".github/copilot-instructions.md"] + list(
    (ROOT / "docs/agent").glob("*.md")
):
    if not path.exists():
        continue
    text = read(path)
