...
Expected: `py_test` fails because a non-owner can read another user's invoice. Semgrep flags the risky `return user_id is not None` flow and the broader helper shape that rejects anonymous users without returning a `user_id == invoice.owner_id` owner check.
